...
/**
* Represents the main TheDiscord client
...
* @prop {Object} options TheDiscord options
...
* @prop {Collection<Shard>} shards Collection of shards TheDiscord is using
...
class Client extends EventEmitter {
    ...
    * @arg {Object} options TheDiscord client options
    ...
    * @arg {Boolean} [options.autoreconnect=true] Have TheDiscord autoreconnect when connection is lost
    ...
    * @arg {Object} [options.disableEvents] If disableEvents[eventName] is true, the WS event will not be processed. This can cause significant performance increase on large bots. [A full list of the WS event names can be found on the docs reference page](/TheDiscord/docs/reference#ws-event-names)
    ...
    * @arg {Number | Array<String|Number>} [options.intents] A list of [intent names](/TheDiscord/docs/reference), pre-shifted intent numbers to add, or a raw bitmask value describing the intents to subscribe to. Some intents, like `guildPresences` and `guildMembers`, must be enabled on your application's page to be used. By default, all non-privileged intents are enabled.
    ...
    * @arg {Number} [options.latencyThreshold=30000] [DEPRECATED] The average request latency at which TheDiscord will start emitting latency errors. This option has been moved under `options.rest`
    ...
    * @arg {Number | String} [options.maxShards=1] The total number of shards you want to run. If "auto" TheDiscord will use Discord's recommended shard count.
    ...
    * @arg {Number} [options.rest.latencyThreshold=30000] The average request latency at which TheDiscord will start emitting latency errors
    ...
    * @arg {Number | String} [options.shardConcurrency="auto"] The number of shards that can start simultaneously. If "auto" TheDiscord will use Discord's recommended shard concurrency.
    ...
    */
    getRESTChannel(channelID) {
        if(!this.options.restMode) {
            return Promise.reject(new Error("TheDiscord REST mode is not enabled"));
        }
